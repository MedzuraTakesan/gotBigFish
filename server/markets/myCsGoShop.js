...
const init = (handler) => {
    let ws = new WebSocket('wss://mycsgo.shop/websocket');

    webSocket = ws
    handleMessage = handler

    ws.onopen = function(socket){
        console.log(`${MARKETS.MY_CSGO_SHOP} connected`)
    ...
    ws.on('error', function() {
        console.log(`${MARKETS.MY_CSGO_SHOP} error`)
    });

    ws.on('close', function() {
        setTimeout(() => {
            console.log(`${MARKETS.MY_CSGO_SHOP} close`)
            init(handler)
        }, 5000);
    });
}
